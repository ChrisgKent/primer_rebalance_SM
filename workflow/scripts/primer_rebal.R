...
n_sm = snakemake@params[["n"]] %>% 
  as.numeric()
ul_of_each_primer = snakemake@params[["ul_of_each_primer"]] %>% 
  as.numeric()
...
vol_cal = function(x){mutate(x,ul_of_each_primer,
                             amplicon_added = normalised_n*ul_of_each_primer*2,
...
output = cbind(vol_added_1,vol_added_2) %>%
  .[,c(1,2,5,6,7,8,10,11)] 
names(output) = c("amplicon_1","median_coverage_prop", "ul_of_each_primer_1","pcr_conc_nM_1", "amplicon_2","median_coverage_prop_2", "ul_of_each_primer_2","pcr_conc_nM_2")
